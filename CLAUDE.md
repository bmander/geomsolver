...
Currently: **Stage 2 done** — vectorized numpy kernels (`gcs.kernels`), compile-to-plan `System`, own DogLeg/LM
(`gcs.newton`), structural diagnosis (`gcs.graph`, `gcs.diagnose`); scipy kept only as `scipy-*` reference methods.
Python ≥ 3.14 in `.venv/`.
