...
- **[0.8] A joint may state a list.** `A -> (equal, angle(30deg)) B` states both relations between the two links, at the corner the marker threads; each member desugars to a statement of its own, with its own identity and span. The marker may stand on either side of the words, or both — `A -> (equal) -> B` is the one joint `A -> equal B` is — and a list with no marker states its relations and welds nothing. The list is read leniently (a trailing comma, or an empty `()`, is a plain corner's worth of nothing) because deleting a member splices it out of the list and leaves the rest standing. After a DECLaration a bare `(` is that declaration's own argument list, so an unthreaded list there needs the marker (or the longhand); between two REFs it reads as the joint it is.
