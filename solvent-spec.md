...
- **Threading.** **[0.8]** A link a marker reaches is a line or an arc — an element with an entry and an exit, read left to right (`p1 → p2`; CCW, `start → end`); a kind with no boundary points — a circle — cannot be reached by a marker, though it may stand in a chain no marker touches. At each `->` the shared point MUST be named by exactly one side, or by both in agreement; the name fills whichever boundary field a declared side left out. A link that only *names* an element offers no list to read or fill — its boundary is its own declaration's business — so at a corner with one, the declared side MUST name the shared point, usually by the existing element's own child (`k.start`). An end no marker filled, on a link some marker reached, MUST be named where it stands, or its child list is left partial (§6.1).
