...
def test_triangle_has_two_roots_and_the_other_can_be_applied() -> None:
    ...
    apply_alternative(ps.plan, idx, alts[1])
    assert c.y.value < 0
    ps.solve()
    assert c.y.value < 0 and all(k.error() < 1e-6 for k in sk.hard_constraints())


@pytest.mark.skipif(os.environ.get("GCS_SLOW") != "1", reason="~20 s (256 homotopy paths); set GCS_SLOW=1")
def test_k33_core_has_several_real_realizations() -> None:
    sk = examples.k33()
    ps = PlanSolver(sk, sticky=True)
    ps.solve()
    idx = max(range(len(ps.plan.steps)), key=lambda i: len(ps.plan.steps[i].ids))
    assert len(ps.plan.steps[idx].ids) == 9
    alts = enumerate_step(ps.plan, idx)
    assert len(alts) >= 2 and any(a.is_current for a in alts)
    x0 = sk.get_x()
    apply_alternative(ps.plan, idx, next(a for a in alts if not a.is_current))
...
def test_under_determined_merges_are_skipped() -> None:
    ...
    for i in range(len(ps.plan.steps)):
        enumerate_step(ps.plan, i, max_paths=16)
